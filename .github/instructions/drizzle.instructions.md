...
- API comments must state the function's purpose, describe every parameter (including the injectable `db` argument), and describe the return value. Use `@param` and `@returns` when they make the contract clearer.
- Keep comments focused on the public contract and non-obvious decisions; do not restate the implementation. Update or remove comments whenever the related behavior changes.
